...
body{font-family:Gill Sans MT;}fieldset{border:solid 1px #000;padding:10px;display:block;clear:both;margin:5px 0}legend{padding:0 10px;background:#000;color:#fff}label.numAds{float:left;display:block;margin:5px;}
  label.bType,label.fBSrcs,label.fAffLnk{float:left;display:block;margin:5px;clear:both;}.add{float:right;display:block;margin:5px;clear:both;}
  input.bType,input.fBSrcs,input.fAffLnk,input.remove,textarea.bType,textarea.fBSrcs,textarea.fAffLnk,select.bType,select.fBSrcs,select.fAffLnk{float:left;display:block;margin:5px;}
          ...
          adS = parseInt($( "#numAds_s option:selected" ).val());  
            ...
            if (newarr_h != "" && newarr_v != "" && newarr_s != ""){
               ...
               $('#outputhtml').text(newarrDivHTML.join("\n"));
                ...
                $('#outputjs').text("<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.js'><\/script>\n<script type='text/javascript'>\n$(window).bind(\"load\",function(){function rnumG(r,n,t){for(var a=r,e=n,o=t,u=new Array;u.length<a;){var f=Math.floor(Math.random()*(o-e)+.5)+e;-1==u.indexOf(f)&&u.push(f)}return u.sort(function(r,n){return r-n}),u}var arr_h=[" + newarr_h + "],rnumH=rnumG("+adH+",0,arr_h.length-1)," + newarrSrc_h + ";" + newarrDiv+"});\n<\/script>"); 
              	$('#outputhtml').text(newarrDivHTML.join("\n"));
            }else if (newarr_h == "" && newarr_v != "" && newarr_s == ""){
                $('#outputjs').text("<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.js'><\/script>\n<script type='text/javascript'>\n$(window).bind(\"load\",function(){function rnumG(r,n,t){for(var a=r,e=n,o=t,u=new Array;u.length<a;){var f=Math.floor(Math.random()*(o-e)+.5)+e;-1==u.indexOf(f)&&u.push(f)}return u.sort(function(r,n){return r-n}),u}var arr_v=[" + newarr_v + "],rnumV=rnumG("+adV+",0,arr_v.length-1)," + newarrSrc_v + ";" + newarrDiv+"});\n<\/script>"); 
              	$('#outputhtml').text(newarrDivHTML.join("\n"));
            }else if (newarr_h == "" && newarr_v == "" && newarr_s != ""){
                $('#outputjs').text("<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.js'><\/script>\n<script type='text/javascript'>\n$(window).bind(\"load\",function(){function rnumG(r,n,t){for(var a=r,e=n,o=t,u=new Array;u.length<a;){var f=Math.floor(Math.random()*(o-e)+.5)+e;-1==u.indexOf(f)&&u.push(f)}return u.sort(function(r,n){return r-n}),u}var arr_s=[" + newarr_s + "],rnumS=rnumG("+adS+",0,arr_s.length-1)," + newarrSrc_s + ";" + newarrDiv+"});\n<\/script>"); 
              	$('#outputhtml').text(newarrDivHTML.join("\n"));
            }else{
...
<fieldset id="buildyourform"><legend>Generate Your Dynamic Ads!</legend>Number of Ads to display:<p/><label for="numAds_h">horizontal:</label><select class="numAds" id="numAds_h"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select> <label for="numAds_v">vertical: </label><select class="numAds" id="numAds_v"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select> <label for="numAds_s">square: </label><select class="numAds" id="numAds_s"><option value="0">0</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select></fieldset><input class="add" id="add" type="button" value="Add Affiliate Link" /><input class="add" id="gencode" type="button" value="Generate Code" /><!--<input type="button" value="Preview form" class="add" id="preview" />--> <br /><div></div><fieldset class="outp" id="opcodediv"><legend>Javascript/HTML Code</legend><label for="outputjs" class="opcode">Javacript:</label><button class="btn btn-sm btn-secondary copytoclipboard" data-clipboard-action="copy" data-clipboard-target="#outputjs" id="copytoclipboard" type="button"><i class="fa fa-copy"></i></button><textarea class="outp" cols="50" id="outputjs" rows="10" readonly></textarea>  <label for="outputhtml" class="opcode">HTML:</label><button class="btn btn-sm btn-secondary copytoclipboard" data-clipboard-action="copy" data-clipboard-target="#outputhtml" id="copytoclipboard" type="button"><i class="fa fa-copy"></i></button><textarea class="outp" cols="50" id="outputhtml" rows="5" readonly></textarea> </fieldset>
